...
public class MessageListPanelEx {
    ...
    private Observer<IMMessage> deleteMsgSelfObserver = new Observer<IMMessage>() {
        @Override
        public void onEvent(IMMessage message) {
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void loadFromRemote() {
            ...
            NIMClient.getService(MsgService.class)
                    .pullMessageHistory(anchor(), loadMsgCount, true, MessageListPanelEx.this.persistClear)
                    .setCallback(callback);
        }
    ...
    private class MsgItemEventListener extends MsgAdapter.BaseViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            if (enableRevokeButton(selectedItem)) {
                ...
                longClickRevokeMsg(selectedItem, alertDialog, false);
            }
            // 4 delete
            ...
            // 9 delete msg self
            longClickItemDeleteMsgSelf(selectedItem, alertDialog);
        }
        ...
        private void longClickItemDeleteMsgSelf(IMMessage selectedItem, CustomAlertDialog alertDialog) {
            if (recordOnly) {
                return;
            }
        ...
        // 长按菜单项 -- 撤回消息
        private void longClickRevokeMsg(final IMMessage item, CustomAlertDialog alertDialog, boolean shouldNotifyBeCount) {
            alertDialog.addItem(container.activity.getString((shouldNotifyBeCount ? R.string.withdrawn_msg_count_notify : R.string.withdrawn_msg_not_count_notify)), new CustomAlertDialog.onSeparateItemClickListener() {
                @Override
                public void onClick() {
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
                    NIMClient.getService(MsgService.class).revokeMessage(item, "撤回一条消息", payload, shouldNotifyBeCount).setCallback(new RequestCallback<Void>() {
                        @Override
